<?php /*
  ...
  Plugin URI:   http://classipro.es/
  Description:  Showing related ads thumbnails under the Ad.
  Version:      1.3.3
  Author:       Alucas & Rubencio
  Author URI:   http://classipro.es/
*/
/*  Copyright 2010  Apptheming
...
*/
define('TIMTHUMB', plugins_url('/related-ads/timthumb.php'));
class RelatedPostsThumbnails {
	/* Default values. PHP 4 compatible */
	var $single_only = '1';
	var $auto = '1';
	var $top_text = '<h3>Related ads:</h3>';
	var $number = 3;
	...
	var $post_types = array('ad_listing'); // mod post -> ad_listing
		...
		if ( $use_filter ) {
			...
			if ( $relation != 'no' ) { /* Get object terms */
				if ( $relation == 'categories' )
					$taxonomy = array( 'ad_cat' ); // mod category -> ad_cat
				elseif ( $relation == 'tags' )
					$taxonomy = array( 'ad_tag' ); // mod tag -> ad_tag
				elseif ( $relation == 'custom') {
					$taxonomy = get_option( 'relpoststh_custom_taxonomies', $this->custom_taxonomies );
				}
				else {
					$taxonomy = array( 'ad_cat', 'ad_tag' ); // mod category->ad_cat, post_tag -> ad_tag
				}
		...
		foreach( $posts as $post ) {
			...
			if ( $thsource == 'custom-field' ) {
				...
				if ( $from_post_body ) { // Theme does not support post-thumbnails, or post does not have assigned thumbnail
					 ...
					 */
					$attachments = get_posts(array('post_type' => 'attachment','numberposts' => $this->number,'post_status' => null,'order' => 'ASC','orderby' => 'menu_order','post_mime_type' => 'image','post_parent' => $post->ID));
					$height = 50;
					$width = 50;
					if ($attachments)
						foreach($attachments as $attachment) 
            				$url = TIMTHUMB.'?src='.wp_get_attachment_url($attachment->ID, false).'&h='.$height.'&w='.$width.'&zc=0';
				}
				$basic_url = $url;
	...
	function is_relpoststh_show() { // Checking display options
		...
		if ( $categories_all != '1') { // only specific categories were selected
			$post_categories = wp_get_object_terms( $id, array( 'category' ), array( 'fields' => 'ids' ) ); // mod category ->ad_cat
	...
	function admin_menu() {
		$page = add_options_page( __( 'Related Ads', 'related-posts-thumbnails' ), __( 'Related Ads', 'related-posts-thumbnails' ), 'administrator', 'related-posts-thumbnails', array( $this, 'admin_interface' ) );
	}
...
<div class="wrap">
	...
	<h2><?php _e( 'Related Ads Settings', 'related-posts-thumbnails' ); ?></h2>
...
class RelatedPostsThumbnailsWidget extends WP_Widget {
	function RelatedPostsThumbnailsWidget() {
		parent::WP_Widget(false, $name = 'Related Ads');
	}
