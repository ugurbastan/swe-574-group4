...
if ( !class_exists( 'Simple_Map' ) ) {
	...
	class Simple_Map {
		...
		function location_search_form( $atts ) {
			...
			$ffi['city']		= array( 'label' => apply_filters( 'sm-search-label-city', __( 'Sehir: ', 'SimpleMap' ), $post ), 'input' => '<input type="text" class="text" id="location_search_city_field" name="location_search_city" value="' . esc_attr( $city_value ) . '" />' );
		...
		function add_distance_field( $radius_value, $units ) {
			...
			return array( 'label' => apply_filters( 'sm-search-label-distance', __( 'Yaricap Alani: ', 'SimpleMap' ), $post ), 'input' => $distance_input );
		...
		function google_map_js_script() {
			...
			var visit_website_text 		= '<?php echo apply_filters( 'sm-visit-website-text', __( 'AV Goruntule', 'SimpleMap' ) ); ?>';
			var get_directions_text		= '<?php echo apply_filters( 'sm-get-directions-text', __( 'Yol Tarifi Al', 'SimpleMap' ) ); ?>';
		...
		function get_options() {
			...
			if ( empty( $default ) ) {
				$default = array(
					'map_width' => '100%',
					'map_height' => '350px',
					'default_lat' => '39.92077',
					'default_lng' => '32.854101',
					'zoom_level' => '10',
					'default_radius' => '10',
					'map_type' => 'ROADMAP',
					'special_text' => '',
		...
		function get_search_radii(){
			$search_radii = array( 1, 5, 10, 25, 50 );
		...
		function get_default_shortcode_atts() {
			...
			$atts = $tax_atts + array(
				'search_title'				=> __( 'Haritada Loksayona Gore Arama Yap:', 'SimpleMap' ), 
