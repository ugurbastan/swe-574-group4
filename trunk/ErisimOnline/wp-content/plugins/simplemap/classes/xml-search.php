	...
	class SM_XML_Search{
		...
		function init_search() {
			if ( isset( $_GET['sm-xml-search'] ) ) {
				...
				$distance_select = $wpdb->prepare( "( 3959 * ACOS( COS( RADIANS(%s) ) * COS( RADIANS( lat_tbl.lat ) ) * COS( RADIANS( lat_tbl.lng ) - RADIANS(%s) ) + SIN( RADIANS(%s) ) * SIN( RADIANS( lat_tbl.lat ) ) ) ) AS distance", $input['lat'], $input['lng'], $input['lat'] ) . ', ';
				...
				$sql = $wpdb->prepare( "
					...
					FROM
						$wpdb->posts AS posts
					INNER JOIN
						$wpdb->cp_ad_geocodes lat_tbl ON lat_tbl.post_id = posts.ID
						$taxonomy_join
					WHERE
						posts.post_type = 'ad_listing'
